 ...
 *
 * 光栅化用离屏 2D canvas 的 fillText：永远画白字。彩色字体（Apple Color Emoji /
 * Noto Color Emoji）会无视 fillStyle 画成彩色，扫像素有色度就进 RGBA 图集，否则
 * 取 alpha 通道进 R8——零 Unicode 表，与系统字体回退行为天然一致。
 *
...
/** 有任何非透明像素带色度（通道极差 > threshold）就是彩色字形 */
export function hasChroma(rgba: Uint8ClampedArray | Uint8Array, threshold = 8): boolean {
  for (let i = 0; i < rgba.length; i += 4) {
    if (rgba[i + 3] === 0) continue;
    const r = rgba[i]!;
    const g = rgba[i + 1]!;
    const b = rgba[i + 2]!;
    if (Math.max(r, g, b) - Math.min(r, g, b) > threshold) return true;
  }
  return false;
}

/** 单码位字形的数字 key：cp 占 21 位，样式位往上叠。不做字符串分配 */
export function glyphKey(cp: number, bold: boolean, italic: boolean, wide: boolean): number {
  return (cp & 0x1fffff) | ((bold ? 1 : 0) << 21) | ((italic ? 1 : 0) << 22) | ((wide ? 1 : 0) << 23);
}
...
export interface Rasterizer {
  /** 空 ink 盒返回 null */
  raster(text: string, bold: boolean, italic: boolean, wide: boolean): Raster | null;
  dispose(): void;
}
...
export class GlyphAtlas {
  ...
  /**
   * 单码位走数字 key；cluster（ZWJ emoji、组合附标）走字符串 key。
   * 返回 id ≥ 0，或 GLYPH_EMPTY（没有 ink，不发实例）/ GLYPH_FULL（图集满，调用方 reset 后重来）
   */
  get(cp: number, text: string | null, bold: boolean, italic: boolean, wide: boolean): number {
    if (text === null) {
      const key = glyphKey(cp, bold, italic, wide);
      const hit = this.byKey.get(key);
      if (hit !== undefined) return hit;
      const id = this.insert(String.fromCodePoint(cp), bold, italic, wide);
      if (id !== GLYPH_FULL) this.byKey.set(key, id);
      return id;
    }
    const key = `${text}|${(bold ? 1 : 0) | (italic ? 2 : 0) | (wide ? 4 : 0)}`;
    const hit = this.byText.get(key);
    if (hit !== undefined) return hit;
    const id = this.insert(text, bold, italic, wide);
  ...
  private insert(text: string, bold: boolean, italic: boolean, wide: boolean): number {
    const raster = this.rasterizer.raster(text, bold, italic, wide);
    if (!raster) return GLYPH_EMPTY;
    return this.place(raster);
  }
