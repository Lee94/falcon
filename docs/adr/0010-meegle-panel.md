...
- **复制工作项 Key**：复制 CLI 的 `work_item_id`，不带 `#` 或其它说明文字。
- **复制 AI 修复上下文**：用户选择后才重新获取详情，复制结构化 Markdown。
  包含来源标识、空间 / 业务线 / 类型、状态 / 节点 / 优先级、时间、原始描述与相关
  缺陷字段；不复制完整人员名单、角色、邮箱或操作历史。获取失败就报错，不拿列表摘要
  冒充完整详情；缺少字段只说明“本次数据未提供”，不声称飞书中未填写。

`description` 继续供面板纯文本预览，`descriptionMarkdown` 保留图片引用与代码块，
两者不能混用。复制内容说明评论、关联工作项全文、图片 / 附件内容未自动获取，来源正文
