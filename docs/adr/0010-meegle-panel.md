...
同一时刻只有一个登录进程：再点一次给同一份链接，别起第二个把第一个的 code 作废。十分钟没授权就把进程收掉。登录进程退出时把用户 / 空间 / 类型缓存都清掉——换人了。
...
视图下钻页（普通视图与全景视图共用）顶上是筛选框 + **分类过滤器**：飞书那边视图左侧的分类树与分组是页面自己的东西，CLI 一条都不给（`view get` / `list-multi-project-workitems` 只回一串工作项，全景视图 URL 里的 `node=` 也没有对应入参），所以不去"把分类找回来"，而是就已加载的条目现推维度——类型 / 状态 / 空间，每维一个下拉，只有一种取值的维度不占位。统计口径是**已加载的条目**，所以配一个「全部加载」把剩下的页翻完（封顶 20 页，防手滑点在几千条的视图上），底下那行"已加载 N / 总数"就是提醒。全景视图那行本来只有 `project_key`，空间名也在 `enrich()` 里补上了（`spaces()` 只有最近访问过的空间，跨到没访问过的就留空）。
...
- 视图自己的分组 / 左侧分类树取不到（CLI 没开出来），面板里只能按已加载条目现推的类型 / 状态 / 空间过滤，见"面板结构"。
- 待办名字要多一轮 MQL，首页 50 条约 2–4 秒；空间搜索按类型扇出，十几个类型的空间 4–6 秒。都是网络往返，没法再省。
...
- 搜索结果封顶 60 条，视图搜索每类型最多 CLI 给多少算多少；面板窄，翻不到几十条以外。
