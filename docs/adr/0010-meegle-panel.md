...
视图下钻页（普通视图与全景视图共用）顶上是筛选框，底下是分组树，见"工作项分组与复制上下文"。飞书那边视图左侧的分类树与分组是页面自己的东西，CLI 一条都不给（`view get` / `list-multi-project-workitems` 只回一串工作项，全景视图 URL 里的 `node=` 也没有对应入参），所以面板里的层级不是"把分类找回来"，是就手上的条目自己推的。全景视图那行本来只有 `project_key`，空间名在 `enrich()` 里补（`spaces()` 只有最近访问过的空间，跨到没访问过的就留空）。
...
- 视图自己的分组 / 左侧分类树取不到（CLI 没开出来），面板里的层级是就手上条目现推的，见"工作项分组与复制上下文"。
